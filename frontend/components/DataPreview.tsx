...
const sourceFields = {
  crm: ["customer_id", "company_name", "industry", "deal_stage", "region", "account_tier"],
  billing: ["plan", "mrr", "payment_status", "billing_cycle", "last_payment"],
  product: ["total_sessions", "usage_pct", "last_active", "features_used", "api_calls"],
  support: ["total_tickets", "nps_score", "avg_resolution_hrs", "last_ticket_date"],
};
...
function extractSourceData(profile: Record<string, unknown>, source: keyof typeof sourceFields) {
...
export default function DataPreview({ result }: DataPreviewProps) {
  ...
  const sources = profile
    ? {
        ...
        billing: extractSourceData(profile, "billing") || defaultSources.billing,
        product: extractSourceData(profile, "product") || defaultSources.product,
        support: extractSourceData(profile, "support") || defaultSources.support,
      }
    : defaultSources;
  ...
  const enrichedData = profile
    ...
    : { customer_id: "—", company_name: "—", plan: "—", mrr: "—", health_score: "—", churn_risk: "—", expansion_potential: "—" };
    ...
    <div className="h-80 overflow-y-auto pr-3">
      ...
      <div className="mb-4">
        <div className="text-xs text-gray-500 mb-2 uppercase tracking-wider">
          {profile ? `Before: Customer ${profile.customer_id}` : "Before: 4 Sources"}
        </div>
